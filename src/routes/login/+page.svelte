<script lang="ts">
  ...
  //TODO: Fix Login in Email and Password
  ...
  onMount(() => {
    const unsubscribe = onAuthStateChanged(auth, (currentUser) => {
      authStore.set(currentUser); // Update authStore
    });
    return () => unsubscribe();
  });
  ...
  async function signInWithEmail() {
    try {
      ...
      goto('/timer'); // Redirect using SvelteKit's goto function
    } catch (error) {
...
{#if $authStore === null} <!-- Use $authStore to get the current value -->
      ...
      <div class="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
          <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
              ...
              <form class="space-y-4 md:space-y-6" action="#">
                  ...
                  <button type="submit" on:click={signInWithEmail} class="w-full text-white bg-primary-600 hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">Sign in</button>
